...
const WRITABLE_SCALAR_FIELDS = ["name", "industry", "description", "goals", "services", "targetAudience", "brandInfo", "logo", "status", "ownerEmail", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["departments", "agents", "tasks", "brainMemories", "workflows", "activities"] as const
...
export function createCompanyRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Company not found" } }, 404)
      }
