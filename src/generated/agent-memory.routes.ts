...
/**
 * Auto-generated AgentMemory Routes
 *
...
import type { AgentMemoryHooks } from "./agent-memory.hooks"
...
/**
 * Set hooks for AgentMemory routes
 */
export function setAgentMemoryHooks(h: AgentMemoryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["content", "category", "agentId", "createdAt"] as const
...
/**
 * Create AgentMemory CRUD routes
 */
export function createAgentMemoryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AgentMemory] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.agentMemory.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AgentMemory not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AgentMemory] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.agentMemory.create({
        data: picked.data,
      })
      ...
      console.error("[AgentMemory] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.agentMemory.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AgentMemory] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AgentMemory] Delete error:", error)
