...
/**
 * Auto-generated AgentMessage Routes
 *
...
import type { AgentMessageHooks } from "./agent-message.hooks"
...
/**
 * Set hooks for AgentMessage routes
 */
export function setAgentMessageHooks(h: AgentMessageHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["content", "role", "agentId", "createdAt"] as const
const RELATION_FIELDS = ["agent"] as const
...
/**
 * Create AgentMessage CRUD routes
 */
export function createAgentMessageRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[AgentMessage] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.agentMessage.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "AgentMessage not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[AgentMessage] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.agentMessage.create({
        data: picked.data,
      })
      ...
      console.error("[AgentMessage] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.agentMessage.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[AgentMessage] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[AgentMessage] Delete error:", error)
