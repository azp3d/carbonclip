...
/**
 * Auto-generated BrainMemory Routes
 *
...
import type { BrainMemoryHooks } from "./brain-memory.hooks"
...
/**
 * Set hooks for BrainMemory routes
 */
export function setBrainMemoryHooks(h: BrainMemoryHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["content", "category", "companyId", "createdAt"] as const
const RELATION_FIELDS = ["company"] as const
...
/**
 * Create BrainMemory CRUD routes
 */
export function createBrainMemoryRoutes(): Hono {
  ...
  router.get("/", async (c) => {
    try {
      ...
      console.error("[BrainMemory] List error:", error)
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.brainMemory.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "BrainMemory not found" } }, 404)
      }

      return sendJson(c, { ok: true, data: item })
    } catch (error: any) {
      console.error("[BrainMemory] Get error:", error)
  ...
  router.post("/", async (c) => {
    try {
      ...
      const item = await prisma.brainMemory.create({
        data: picked.data,
      })
      ...
      console.error("[BrainMemory] Create error:", error)
  ...
  router.patch("/:id", async (c) => {
    try {
      ...
      const item = await prisma.brainMemory.update({
        where: { id },
        data: picked.data,
      })
      ...
      console.error("[BrainMemory] Update error:", error)
  ...
  router.delete("/:id", async (c) => {
    try {
      ...
      console.error("[BrainMemory] Delete error:", error)
