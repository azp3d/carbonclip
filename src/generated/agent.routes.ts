...
/**
 * Set hooks for Agent routes
 */
export function setAgentHooks(h: AgentHooks) {
  hooks = h
}
...
const WRITABLE_SCALAR_FIELDS = ["name", "role", "personality", "skills", "experienceLevel", "performanceScore", "status", "systemPrompt", "avatar", "departmentId", "companyId", "createdAt", "updatedAt"] as const
const RELATION_FIELDS = ["department", "company", "tasks", "messages", "memories"] as const
...
export function createAgentRoutes(): Hono {
  ...
  router.get("/:id", async (c) => {
    try {
      ...
      const item = await prisma.agent.findUnique({
        where: { id },
      })

      if (!item) {
        return sendJson(c, { error: { code: "not_found", message: "Agent not found" } }, 404)
      }
